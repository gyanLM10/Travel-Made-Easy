...
def render_plan(question: str, result: dict, source_label: str = "") -> None:
    ...
    title = f"🌍 AI Travel Plan{f' ({source_label})' if source_label else ''}"
    st.markdown(
        f"""
# {title}
...
"""
    ...
    st.divider()
    st.subheader("🔍 Trustworthiness Report")

    if score == -1:
        st.info("ℹ️ Confidence check was unavailable for this response.")
    else:
        # Colour-coded badge
        if score >= 80:
            badge = f"🟢 **High Confidence — {score}/100**"
            st.success(badge)
        elif score >= 50:
            badge = f"🟡 **Verify Before Booking — {score}/100**"
            st.warning(badge)
        else:
            badge = f"🔴 **Low Confidence — {score}/100**"
            st.error(badge)
        ...
        with col1:
            if uncertain:
                with st.expander(f"⚠️ {len(uncertain)} claim(s) to verify", expanded=score < 60):
                    for claim in uncertain:
                        st.markdown(f"- {claim}")
            else:
                st.success("✅ No specific unverifiable claims flagged.")

        with col2:
            if verified:
                with st.expander("✅ Grounded by tools", expanded=False):
...
if submit_button and user_input.strip():
    ...
    with st.spinner("🔍 Running trustworthiness check..."):
